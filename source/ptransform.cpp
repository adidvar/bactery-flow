...
number PTransformer::transform_p(number current_position,
                                 number max_position,
...
{
  return transform_p(current_position - cposition[0].x,
                     cposition[1].x - cposition[0].x,
                     width);
}

number PTransformer::ScaleH(number current_position) const
{
  return transform_p(current_position - cposition[0].y,
                     cposition[1].y - cposition[0].y,
                     height);
}
...
{
  number dx = cposition[1].x - cposition[0].x,
         dy = cposition[1].y - cposition[0].y;
